...
void* sb_parse_http_start(sb_client *client, sb_request *request, void *args){
    ...
    parse_get_request_parameters(request);
...
void* sb_parse_http_head(sb_client *client,sb_request *request,void *args){
    ...
    sb_set_request_parameter(request,REQUEST_HEADS,heads);
    return first_cursor;
}
...
void* sb_parse_http_body(sb_client *client,sb_request *request,void *args){
    char *first_cursor = (char*)args;
    const char *request_method = sb_get_key_value(&request->request_data,REQUEST_METHOD);
    if(strcmp(request_method,"GET") == 0){
        //这是一个GET请求
        const char *target = sb_get_request_parameter(request,REQUEST_TARGET);
